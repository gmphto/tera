...
| Owning issue | [#76](https://github.com/gmphto/tera/issues/76) — the producer labeling session re-run against the #16 protocol (filed as the replacement for the closed [#18](https://github.com/gmphto/tera/issues/18)), on [#65](https://github.com/gmphto/tera/issues/65)'s pair list with [#66](https://github.com/gmphto/tera/issues/66)'s evaluators |
| Owner role | Producer panel (the session itself); orchestrator for briefing and for follow-up ownership; user for credentials |
| Recorded inputs it needs | `PROTOCOL_VERSION = tera-eval-protocol-v1`; dataset version `fca713b6e113df4d83684f3c630e42602c0dee34b99f45c95022d6fb169d13f5`; the split-manifest digest and pair-list digest from #65, both `not recorded` today; the held-out analysis manifests from [#77](https://github.com/gmphto/tera/issues/77); a rating session with ≥ `MIN_EVALUATORS = 5` counted evaluators and ≥ `MIN_RATINGS_PER_PAIR = 2` valid ratings per pair, over ≥ `MIN_HELDOUT_QUERIES = 60` eligible queries and ≥ `MIN_HELDOUT_PAIRS = 300` pairs; live-Jev prerequisites from #68 / #14 — `TERA_JEV_ENDPOINT` and `TERA_JEV_API_KEY` set, so that ≥ 1 `source="interface"` outcome exists |
...
| No validated rating session: 0 valid ratings consumed; #18 is closed with no session artifact, and the re-run is filed as #76 | Producer panel; orchestrator owns the re-run | One session whose `validate` exit status is zero, 0 excluded sessions, and ≥ `MIN_RATINGS_PER_PAIR = 2` valid ratings per pair | [#76](https://github.com/gmphto/tera/issues/76) (replaces the closed [#18](https://github.com/gmphto/tera/issues/18)) |
...
| No analysis manifest: 2 of 2 declared analysis manifests are absent — `analysis_manifest_missing`; #9 is closed, and the follow-up is filed as #77 | Engineer | One complete analysis manifest per held-out sample used, each with a single `analysis_version` | [#77](https://github.com/gmphto/tera/issues/77) (replaces the closed [#9](https://github.com/gmphto/tera/issues/9)) |
...
Every deferred prerequisite above links an open issue: #76 replaces the closed #18 for the producer session and #77 replaces the closed #9 for the analysis manifests.
...
- The pair-rating utility, playback, resume and export: [#17](https://github.com/gmphto/tera/issues/17); the producer session: [#76](https://github.com/gmphto/tera/issues/76), filed as the replacement for the closed [#18](https://github.com/gmphto/tera/issues/18); recruitment beyond the current panel: [#66](https://github.com/gmphto/tera/issues/66).
